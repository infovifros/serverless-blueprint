...
async function invokeWithWarmPing(qualifiedFunctionName: string): Promise<void> {
  ...
  try {
    logger.debug('[warm-lambdas] Pinging function', {functionName: qualifiedFunctionName});
    await lambdaClient.send(invokeCommand);
    logger.info('[warm-lambdas] Ping sent', {functionName: qualifiedFunctionName});
  } catch (invocationError: unknown) {
    // Log but do not rethrow — a single failed ping should not abort the rest.
    logger.error('[warm-lambdas] Failed to ping function', {
      functionName: qualifiedFunctionName,
 ...
 *
 * Triggered by a CloudWatch Events / EventBridge schedule (see `warm-lambdas.yml`).
...
export async function handler(_event: ScheduledEvent, _context: Context, callback: Callback): Promise<void> {
  logger.info('[warm-lambdas] Starting warm-up cycle', {functionCount: FUNCTIONS_TO_WARM.length});
  ...
  for (const shortFunctionName of FUNCTIONS_TO_WARM) {
    ...
    logger.info('[warm-lambdas] Processing function', {qualifiedName});
    await invokeWithWarmPing(qualifiedName);
  }

  logger.info('[warm-lambdas] Warm-up cycle complete');
